        ...
        if 'structAcronym' not in doc['_source']:
            for doc_struct in Struct:
                if doc_struct['_source']['structSirene'] == doc['_source']['structSirene']:
                    ...
                    print(doc['_index'])
                    es.update(index=doc['_index'], refresh='wait_for', id=doc['_source']['halStructId'], body={"doc": {"structAcronym": structAcronym}})
                    print(f"le champ structAcronym: {structAcronym} a été rajouté a {doc['_index']}")
        else:
