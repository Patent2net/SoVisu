...
scope_param = scope_all()
res = es.search(
    index="*-structures", body=scope_param
...
for structu in machin:
    ...
    OSclient.indices.create(index_name)
    print("creation ", index_name)
    response = OSclient.index(
        index=index_name, body=structu["_source"], id=id, refresh=True
    )
    ...
    for lab in esLaboratories:
        print(lab)
        # lab="527028"
        OSclient.indices.create(lab["_index"])
        response = OSclient.index(
            index=lab["_index"], body=lab["_source"], id=lab["_id"], refresh=True
        )
        ...
        for doc in esDocs:
            cpt += 1
            OSclient.index(
                index=indexLabDoc, body=doc["_source"], id=doc["_id"], refresh=True
            )
        print(cpt, " docs indexés sur ", count)
        # Récup des chercheurs
        indexLabCher = (
            structu["_id"] + "-" + lab["_source"]["halStructId"] + "-researchers"
        )
