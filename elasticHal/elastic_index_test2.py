...
docmap = {
    "properties": {
      ...
      "concepts": { # cencé être remplacé par la Fiche référentiel !!!!
        "properties": {
          "children": {
            "properties": {
              "children": {
      ...
      }, # dans profil ou section identifiants ?
      "structSirene": {
        "type": "text",
        "fields": {
          "keyword": {
...
def create_test_context():
    ...
    #liste des types de documents nécessaires
    type_doc = ["researchers", "publications", "laboratories", "institutions", "expertises"]
...
def indexe_chercheur(idhal):  # self,
    ...
    # Peut être qu'il faudrait structurer mais pas du tout sûr...
    # chercheur["ids"] = dict()
    # chercheur["profil"]["ids"]['halId_s'] = idhal
    # chercheur["ids"] ["aurehalId"] = aurehal
    # chercheur["ids"]["idRef"] = idref
    # chercheur["ids"]["ldapId"] = ldapid
    # chercheur["ids"]["orcId"] = orcid
    # chercheur["ids"]["researchId"] = ""     # NEW
    # chercheur["ids"]["Scopus"] = ""         # NEW
    ...
    chercheur["idhal"] = idhal # sert de clé pivot entre les docs, il faut être surs que ce champ n'existe dans aucune des doc que l'on pourrait indexer
    ...
    chercheur["category"] = "searcher" # me demande si la présence des autres champs ne suffit pas à marquer la distinction

    # add a common SearcherProfile Key who should serve has common key between index
    # chercheur["SearcherProfile"] = [ # pourquoi une liste ici ???
    #     {"halId_s": chercheur["halId_s"], "ldapId": chercheur["ldapId"], "validated_concepts": chercheur["concepts"]}
    # ]

    chercheur["profil"] = dict()

    chercheur["profil"]["competences"] = dict()
    chercheur["profil"]["travaux"] = dict()  # Publications ou autres
    chercheur["profil"]["parcours"] = dict()            # Parcours
    chercheur["profil"]["rattachement"] = dict()


            # compétences déclarées

    # Competences/expertises
    chercheur["profil"]["travaux"]["thèses"] = []       # thèses soutenu, jury, rapporteur, etc.
    chercheur["profil"]["travaux"]["hal"] = []          # publications sur hal
    chercheur["profil"]["travaux"]["brevets"] = []      # brevets
    # on pourrait rajouter les encadrements de mémoire...
    # Parcours
    chercheur["profil"]["competences"]["Aurehal"] = []# compétence alignées sur référentiel Aurehal
    chercheur["profil"]["competences"]["libre"] = []    # mots clés saisis, ex guidingKeywords

    chercheur["profil"]["parcours"]["formation"] = []
    chercheur["profil"]["parcours"]["experience"] = []
    chercheur["profil"]["discipline"]= []  # faudrait faire un choix ici principale/secondaire ? Identifier un référentiel CNU / CNRS ?
    ...
    # Soit on récupère tout et on filtre sur catégorie
    #seule sverzsion faite ci après. on doit pouvoir avec le scope limiter à la cat de doc souhaité
    res = es.search(index="test2", query=scope_param, size=count)
    labo = [truc for truc in  res["hits"]["hits"] if truc ['_source']['category'] == "laboratory" and chercheur["labHalId"] == truc['_id']][0]

    # A faire : sur le même modèle que les concepts, ou les publis (non vérifié/abouti)
    # copier la fiche labo, rajouter champ : "idHal_s" pour que ce soit le doc du chercheur et le champs category (à la création de la fiche labo)
    chercheur["profil"]["rattachement"] = [{ "name": "labActuel",
                                              "parent": labo ['_id']
                                            }]# le labo peut être une liste

    count = es.count(index="test2", query=scope_param)["count"]
    res = es.search(index="test2", query=scope_param, size=count)

    # A faire idem que précémment. Si chercheur appartient à plusieurs institutions alors autant de fiche doc à son nom.
    structur = [truc for truc in  res["hits"]["hits"] if truc ['_source']['category'] == "institution" and  chercheur["structSirene"] == truc['_id']][0]

    chercheur["profil"]["rattachement"]= [{ "name": "structure",
                                              "parent": structur ['_id']
                                            }]#
    if len(chercheur["concepts"])>0:
      lstConcepts = []
      # scope_param = scope_p("id", [exp['id'] for exp in chercheur["concepts"]['children']])
      # scope_param =  {
      #     "terms": {
      #       "chemin": ["domAurehal."+exp['id'] for exp in chercheur["concepts"]['children']]
      #     }
      #   }
      ...
      toutRef = [truc['_source']['chemin'] for truc in
              es.search(index="domaines-hal-referentiel", query={'match_all': {}}, size=count)['hits']['hits']]

      Vu = set()
      pasVu = set()
      idDomainChecheur = [exp ['id'] for exp in chercheur["concepts"]['children']]
      ...
      for fiche in  resDomainesRef ['hits']['hits']:
        ...
        res = es.index(
          index="test2",
          # id=chercheur["halId_s"],
          document=json.dumps(newFiche),
          refresh="wait_for",
        )

      # chercheur["profil"]["competences"] = lstConcepts # Je pense que c'est mal. Seuls les domaines disciplinaires de plus haut niveau de l'arborescence sont rattachés.
        # La bonne méthode hypothétique serait de créée un doc par concept relié à ses sous-concepts
        # le test pour rappatrier l'id précédent se ferait alors sur l'id complète pour truc et non pour sa version splittée
    res = es.index(
        index="test2",
        # id=chercheur["idhal"], laissons elastic gérer les id ????
...
def get_labo_from_csv():
  # fonctionne pas si on lance 2 fois.. pas cherché aussi cf. A Faire ou TODO
    laboratories_list = []
    scope_param = scope_all()
    count = es.count(index="test2", query=scope_param)["count"]
    print(count)
...
def creeFiche(dom, par):
  fiche = dict()
  #fiche['_id'] = dom['id']   # rajout d'un préffixe
  fiche['chemin'] =  "domAurehal." + dom['id']#.replace('.',',')  # donnera le chemin d'accès de l'arborescence. i.e: chercher .shs
...
def indexe_expertises():
  ...
  """
  concept_list=GenereReferentiel(concepts(), "")
...
def collecte_docs(chercheur, overwrite=False):  # self,
    ...
    doc_param = esActions.scope_term_multi ([("idhal", chercheur["idhal"]), ('category', "notice-hal")])
    ...
    for num, doc in enumerate(docs):
        ...
        if doc["docid"] in IddocsExistantes:
            ...
            #
            if doc["MDS"] != [docAncien["MDS"] for docAncien in docsExistantes if docAncien["_id"] == doc["_id"]][0]:
        ...
        # add a common SearcherProfile Key which will use to store additional data from author


        # Create the records of the searchers linked to the document. NON les autres n'ont rien à faire là

        # validated_concepts = "" # C'est quoi ce validated là ???
        ...
        #     # non les concepts sont pas encore attachés au doc (en tous cas on a pas l'interface pour le faire)
    ...
    #
    #         helpers.bulk(es, boutdeDoc, index="test2", refresh="wait_for")
    #     resp = str(len(docs)) + " indexés "
    # else:
    # searcher_param = esActions.scope_p("halId_s", idhal)
    searcher_param = esActions.scope_term_multi ([("idhal", chercheur["idhal"]), ('category', "searcher")])
    searcher_data = es.search(index="test2", query=searcher_param)
    searcher_data = searcher_data["hits"]["hits"][0] # normalement il n'y en a qu'un !
    ...
    for doc in docs:
        ...
        # indexation
        es.options(request_timeout=200, retry_on_timeout=True, max_retries=5).index(refresh=True, routing=1, # pour contrôler la propagation du lien de parenté
                index="test2", document=doc) # ici par rapport au bulk on force l'id du doc indexé ES comme étant celui du doc.
...
def laboratory_concepts(halStructId):
    ...
    res = es.search(index="test2", query=rsr_param)
...
if __name__ == "__main__":
    ...
    scope_searcher = esActions.scope_match_multi([("idhal", "david-reymond"), ('category', "searcher")])
    ...
    scope_notices = esActions.scope_match_multi([("idhal", "david-reymond"), ('category', "notice-hal")])
    ...
    print("normalement 10 docs (MAIS c'est pas bon cf. infra remarques sur les expertises (çà sort d'où ???) !!!", cpt)
