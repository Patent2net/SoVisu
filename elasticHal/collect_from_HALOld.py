            ...
            if isinstance(docs, list):
                ...
                if len(docs)>1:
                    ...
                    for indi in range(int(len(docs) // 50)+1):
                        ...
                        compte += len(boutdeDoc, " indexés intermédiaires : " + searcher["ldapId"])
                        print (compte)
                        time.sleep(2)
                    print (compte, " indexés total")
            else:
        ...
        if isinstance(docs, list):
            if len(docs)>0:
                for indi in range (int(len(docs) // 50)+1):
                    ...
                    time.sleep(1)
                doc_progress_recorder.set_progress(len(docs), sommeDocs, " documents traités et indexés " + searcher['halId_s'])
        else:
